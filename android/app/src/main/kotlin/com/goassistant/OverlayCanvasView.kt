...
class OverlayCanvasView(context: Context) : View(context) {
    ...
    private val linePaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        style       = Paint.Style.STROKE
        color       = 0xCCD4A843.toInt()
        strokeWidth = 2f
    }
    private val labelBgPaint = Paint(Paint.ANTI_ALIAS_FLAG).apply {
        style = Paint.Style.FILL
        color = 0xDD0D1117.toInt()
    }
    ...
    fun setMove(
        ...
        cx        = (screenW * colFrac).toFloat()
        cy        = (screenH * rowFrac).toFloat()
        radius    = (screenW / boardSize * 0.45f).coerceIn(18f, 52f)
        hasDraw   = move != "pass" && move != "resign"
        reasoning = reason
        score     = scoreStr
        winPct    = (winrate * 100).toInt().coerceIn(0, 100)
        invalidate()
    }
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        val cardW = 280f
        val cardH = 110f
        // Position card so it stays on screen
        var cardX = cx - cardW / 2f
        var cardY = cy + radius + 16f
        if (cardY + cardH > height) cardY = cy - radius - cardH - 16f
        cardX = cardX.coerceIn(16f, width - cardW - 16f)

        val rr = RectF(cardX, cardY, cardX + cardW, cardY + cardH)
        canvas.drawRoundRect(rr, 18f, 18f, labelBgPaint)
        ...
        if (reasoning.isNotEmpty()) {
            val short = if (reasoning.length > 38)
                reasoning.take(35) + "…" else reasoning
            canvas.drawText(short, cardX + cardW / 2f, cardY + 103f, mutedPaint)
        }
    }

    // Called externally to reference colFrac/rowFrac from service
    fun setMove(move: String, boardSize: Int, screenW: Int, screenH: Int) {
        setMove(move, boardSize, screenW, screenH, 0.5, 0.5, 0.5, "", "")
    }
}
